...
function CustomDialog({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  if (!open) return null;
  return (
...
export default function RewardFlow() {
  ...
  useEffect(() => {
    const handler = async () => {
      ...
      if (!alreadyClaimed && shouldShowRewardPopup()) {
        setOpen(true);
        localStorage.setItem("tl_last_reward_popup_date", new Date().toISOString().split("T")[0]);
  ...
  const handleSelectGift = async (gift: any) => {
    ...
    toast({ title: `🎁 Bạn đã chọn ${gift.name}`, description: "Hãy đăng nhập để lưu phần thưởng nhé!" });

    const { data: { user } } = await supabase.auth.getUser();

    if (!user) {
      localStorage.setItem("tl_reward_pending", JSON.stringify({
        item_name: gift.name,
    ...
    toast({ title: "🎉 Đã lưu phần thưởng thành công!", description: `Phần thưởng: ${gift.name}` });
    setOpen(false);
  };
  ...
  return (
    <CustomDialog open={open} onClose={() => setOpen(false)}>
      <div className="max-w-md text-center space-y-4">
        <h2 className="text-2xl font-bold text-primary">🎉 Chúc mừng bạn đã hoàn thành chương!</h2>
        <p className="text-muted-foreground">Hãy chọn 1 phần quà dành riêng cho bạn 🎁</p>
        ...
        <p className="text-xs text-gray-500 mt-2">Sau khi chọn quà, bạn hãy đăng nhập để hệ thống lưu phần thưởng nhé 💫</p>
      </div>
    </CustomDialog>
  );
}
