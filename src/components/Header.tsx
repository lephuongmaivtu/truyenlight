            ...
            {genres.length > 0 ? (
              <>
                ...
                <div className="flex flex-col space-y-1 min-w-0">
                  {genres.slice(0, Math.ceil(genres.length / 3)).map((genre) => (
                    <Link
                      key={genre.id}
                      to={`/genres/${genre.slug}`}
                      className="flex items-center text-sm text-foreground hover:text-primary transition-colors px-1 py-[3px] truncate"
                    >
                ...
                <div className="flex flex-col space-y-1 min-w-0">
                  {genres
                    .slice(Math.ceil(genres.length / 3), Math.ceil((genres.length * 2) / 3))
                    .map((genre) => (
                      <Link
                        key={genre.id}
                        to={`/genres/${genre.slug}`}
                ...
                {/* Cột 3 */}
                <div className="flex flex-col space-y-1 min-w-0">
                  {genres.slice(Math.ceil((genres.length * 2) / 3)).map((genre) => (
                    <Link
                      key={genre.id}
                      to={`/genres/${genre.slug}`}
                      className="flex items-center text-sm text-foreground hover:text-primary transition-colors px-1 py-[3px] truncate"
